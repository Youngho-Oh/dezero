...
class Variable :
    ...
    def reshape(self, *shape) :
        if len(shape) == 1 and isinstance(shape[0], (tuple, list)) :
            shape = shape[0]
        return ohzero.functions.reshape(self, shape)

    def transpose(self) :
        return ohzero.functions.transpose(self)
    ...
    def to_cpu(self):
        if self.data is not None:
            self.data = ohzero.cuda.as_numpy(self.data)

    def to_gpu(self):
        if self.data is not None:
            self.data = ohzero.cuda.as_cupy(self.data)
...
class Add(Function) :
    ...
    def backward(self, gy):
        ...
        if self.x0_shape != self.x1_shape :
            gx0 = ohzero.functions.sum_to(gx0, self.x0_shape)
            gx1 = ohzero.functions.sum_to(gx1, self.x1_shape)
        return gx0, gx1
...
def setup_variable() :
    ...
    Variable.__getitem__ = ohzero.functions.get_item
